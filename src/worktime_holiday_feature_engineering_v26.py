...
class TurnoverFeatureEngineering:
    ...
    def create_target_encoding_features_fast(
    ...
    def create_all_features(self, df):
        ...
        print("ターゲットエンコーディング特徴量を作成中（高速版）...")
        if hasattr(self, "_mode") and self._mode == "inference":
            # 推論モード
            df = self.create_target_encoding_features_fast(
                df, mode="inference", encoding_map=self._encoding_map
            )
        else:
            # 学習モード
            df, encoding_map = self.create_target_encoding_features_fast(
                df, mode="train"
            )
