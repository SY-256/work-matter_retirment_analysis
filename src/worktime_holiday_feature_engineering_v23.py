...
class TurnoverFeatureEngineering:
    ...
    def create_target_encoding_features(
        ...
        if mode == "train":
            ...
            # 1. 部署別ターゲットエンコーディング（Leave-One-Out方式）
            result_df["dept_target_encoded"], encoding_map["department"] = (
                self._target_encode_with_map(
                    result_df, "department", target_col, smoothing=10.0, mode="train"
                )
            )

            # 2. 職位別ターゲットエンコーディング
            result_df["position_target_encoded"], encoding_map["position"] = (
                self._target_encode_with_map(
                    result_df, "position", target_col, smoothing=5.0, mode="train"
                )
            )

            # 3. 年齢グループ別ターゲットエンコーディング
            result_df["age_group"] = pd.cut(
                result_df["age"],
                bins=[20, 30, 40, 50, 65],
                labels=["20s", "30s", "40s", "50s+"],
            )
            result_df["age_group_target_encoded"], encoding_map["age_group"] = (
                self._target_encode_with_map(
                    result_df, "age_group", target_col, smoothing=3.0, mode="train"
                )
            )

            # 4. 勤続年数グループ別ターゲットエンコーディング
            result_df["tenure_group"] = pd.cut(
                result_df["tenure_years"],
                bins=[0, 3, 7, 15, 30],
                labels=["新人", "中堅", "ベテラン", "シニア"],
            )
            result_df["tenure_group_target_encoded"], encoding_map["tenure_group"] = (
                self._target_encode_with_map(
                    result_df, "tenure_group", target_col, smoothing=3.0, mode="train"
                )
            )
            ...
            (
                result_df["dept_size_group_target_encoded"],
                encoding_map["dept_size_group"],
            ) = self._target_encode_with_map(
                result_df, "dept_size_group", target_col, smoothing=5.0, mode="train"
            )
            ...
            # 推論時：事前に作成されたエンコーディングマップを使用
            if encoding_map is None:
                raise ValueError("推論時にはencoding_mapが必要です")

            # 1. 部署別適用
            result_df["dept_target_encoded"] = self._apply_encoding_map(
                result_df, "department", encoding_map["department"]
            )

            # 2. 職位別適用
            result_df["position_target_encoded"] = self._apply_encoding_map(
            ...
            result_df["age_group_target_encoded"] = self._apply_encoding_map(
            ...
            result_df["tenure_group_target_encoded"] = self._apply_encoding_map(
                result_df, "tenure_group", encoding_map["tenure_group"]
            )

            # 5. 部署サイズグループ別適用（推論時は学習時の部署サイズ情報を使用）
            ...
            result_df["dept_size_group_target_encoded"] = self._apply_encoding_map(
    ...
    def create_all_features(self, df):
        ...
        if hasattr(self, "_mode") and self._mode == "inference":
            # 推論モード
            df = self.create_target_encoding_features(
                df, mode="inference", encoding_map=self._encoding_map
            )
        else:
            # 学習モード
            df, encoding_map = self.create_target_encoding_features(df, mode="train")
...
if __name__ == "__main__":
    ...
    print("\n新部署（学習時に見なかった部署）の処理:")
    new_dept_mask = test_df_with_features["department"] == "新部署"
    if new_dept_mask.any():
        print(
            f"新部署のdept_target_encoded: {test_df_with_features.loc[new_dept_mask, 'dept_target_encoded'].iloc[0]:.4f}"
        )
        print(f"全体平均: {encoding_map['department']['global_mean']:.4f}")
        print("→ 学習時に見なかった部署は全体平均で処理されています")
